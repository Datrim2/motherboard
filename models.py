...
class Specifications(Base):
    ...
    picture = Column('Picture', LargeBinary, nullable=False)
    desc_rus = Column('desc_rus', String(2000))
...
class Specifications_corsair(Base):
    ...
    ledLight = Column('LED Lighting', String(200))
    singmultiZone = Column('Single_Multi_Zone', String(200))
    pmicT = Column('PMIC_Type', String(200))
...
class Specifications_Gskill(Base):
    __tablename__ = 'Specifications_Gskill'
    id = Column('id', Integer(), primary_key=True, autoincrement=True)
    code = Column('code', String(200), nullable=False)
    name = Column('name', String(200), nullable=False)
    description = Column('description', String(200))
    menType = Column('Memory Type', String(200))
    capacity = Column('Capacity', String(200))
    multi = Column('Multi-Channel', String(200))
    testSpeed = Column('Tested Speed', String(200))
    testLatency = Column('Tested Latency', String(200))
    testVolt = Column('Tested Voltage', String(200))
    register = Column('Registered/Unbuffered', String(200))
    err = Column('Error Checking', String(200))
    spdSpeed = Column('SPD Speed', String(200))
    spdVolt = Column('SPD Voltage', String(200))
    fan = Column('Fan Included', String(200))
    warranty = Column('Warranty', String(200))
    features = Column('Features', String(200))
    addNote = Column('Additional Notes', String(200))
    formFact = Column('Form Factor', String(200))
    channelConf = Column('Channel Config', String(200))
    spdLat = Column('SPD Latency', String(200))
    casLat = Column('CAS Latency', String(200))
    dramVolt = Column('DRAM Voltage', String(200))

class images_Gskill(Base):
    __tablename__ = 'Images_Gskill'
    id = Column('id', Integer(), primary_key=True, autoincrement=True)
    spec_id = Column('spec_gskill', ForeignKey("Specifications_Gskill.id"))
    img = Column('image', LargeBinary)

class GskillAll(Base):
    __tablename__ = 'gskill'
    id = Column('id', Integer(), primary_key=True, autoincrement=True)
    code = Column('code', String(200), nullable=False)
    url = Column('url', String(2000))
    status = Column('status', Integer())
    
class Specifications_Exceleram(Base):
    __tablename__ = 'Specifications_Exceleram'
    id = Column('id', Integer(), primary_key=True, autoincrement=True)
    name = Column('name', String(200), nullable=False)
    code = Column('code', String(200), nullable=False)
    capaciry = Column('Capacity', String(200))
    speed = Column('Speed', String(200))
    latency = Column('Latency', String(200))
    volt = Column('Volt', String(200))
    dop = Column('Dop', String(200))
    memType = Column('Memory Type', String(20))
    packMemFormat = Column('Package Memory Format', String(20))

class Images_Exceleram(Base):
    __tablename__ = 'Images_Exceleram'
    id = Column('id', Integer(), primary_key=True, autoincrement=True)
    spec_id = Column('spec_Exceleram', ForeignKey("Specifications_Exceleram.id"))
    img = Column('image', LargeBinary)